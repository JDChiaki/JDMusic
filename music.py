...
async def serch_song(song):
    song = "https://www.youtube.com/results?search_query" + urllib.parse.urlencode({"": song})
    url = urllib.request.urlopen(song)
...
class Player(commands.Cog):
    ...
    async def mention(self, ctx, mention, count):
        if int(count) > 10:
